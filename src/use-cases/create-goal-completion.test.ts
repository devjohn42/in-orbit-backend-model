...
describe('create goal completion', () => {
	it('should be able to create a goal completion', async () => {
	...
	it('should increase user experience by 3 when completing- a goal', async () => {
		...
		expect(userOnDb.experience).toEqual(3)
	})

	it('should increase user experience by 5 when fully completing a goal', async () => {
		const user = await makeUser({ experience: 0 })
		const goal = await makeGoal({ userId: user.id, desiredWeeklyFrequency: 1 })
